...
/**
 * The {@link Property} class constitutes the building block of any odML file
 * that actually stores the information. A {@link Property} must have a name and
 * at least one {@link Value}.
 * 
 * <ol>
 * <li>name - mandatory, the name of the property.</li>
 * <li>value - mandatory, its value.</li>
 * <li>definition - optional, a descriptive text that defines the meaning of
 * this property.</li>
 * <li>dependency - optional, mainly used in the terminologies to help GUIs and
 * editors to assist the user entering values. The dependency defines if this
 * property should occur only under certain conditions. I.e. this property
 * occurs, makes sense, only if there also is a certain other property.</li>
 * <li>dependencyValue - optional, as the dependency entry: defines that this
 * property is commonly depends on a certain parent property that contains a
 * specific parent value.</li>
 * <li>mapping - optional</li>
 * <li>guiHelper - optional, additional class to help with GUI</li>
 * </ol>
 * 
 * @author Jan Grewe, Christine Seitz
 * @since 08.2009
 */
public class Property extends Object implements Serializable, Cloneable {
	...
	/** The definition. */
	private String name = "", dependency = "", dependencyValue = "",
			definition = "";
	...
	/** The columns. */
	public static Object[] columns = { "name", "reference", "value",
			"uncertainty", "unit", "type", "filename", "valueDefinition",
			"propertyDefinition", "dependency", "dependencyValue" };

	/** The match exact. */
	public static int MATCH_ERROR = -1, MATCH_NO = 0,
			MATCH_FIRST_CONFLICT_LAST_MATCH = 5, MATCH_INITIALS_ONLY = 10,
			MATCH_FIRST_OR_LAST_ONLY = 20, MATCH_FIRST_INITIAL_LAST = 30,
			MATCH_FIRST_LAST = 50, MATCH_EXACT = 50;

	/*
	 * Constructor to create a NULL type property. The behavior of such a
	 * property can later be modified by setType() method.
	 */
	/**
	...
	/**
	 * Default constructor for creating a property. Only name and the value must
	 * be given.
	 * 
	 * @param name
	 *            {@link String}: the name of the new property, mandatory
	 * @param value
	 *            {@link Object}: the value the property shall contain,
	 *            mandatory except for terminologies
	 * @throws Exception
	 *             the exception
	 */
	 ...
	 *             the exception
	 */
	public Property(String name, Object value, String unit, Object uncertainty,
			String type) throws Exception {
		this(name, null, value, unit, uncertainty, type, null, null, null,
				null, null, null);
	}

	/**
	 * Creates a Property from a Vector containing the property data in the
	 * following sequence:
	 * "name","reference","value","unit","uncertainty","type"
	 * ,"fileName","valueDefinition","propertyDefinition",
	 * "dependency","dependencyValue", "synonym","mappingURL".
	 * 
	 * @param data
	 *            {@link Vector} of Objects that contains the data in the
	 *            sequence as the {@link Property}.columns
	 * @throws Exception
	 *             the exception
	 */
	public Property(Vector<Object> data) throws Exception {
		this((String) data.get(0), (String) data.get(1), data.get(2),
				(String) data.get(3), data.get(4), (String) data.get(5),
				(String) data.get(6), (String) data.get(8), (String) data
						.get(7), (String) data.get(9), (String) data.get(10),
				(URL) data.get(11));
	}
	 ...
	 *            {@link String}: the name of the new property, mandatory
	 * @param values
	 *            {@link Vector}<Value>: the values the property shall contain,
	 *            mandatory except for terminologies
	 * @param definition
	 *            {@link String}
	 * @param dependency
	 ...
	 *             the exception
	 */
	public Property(String name, Vector<Value> values, String definition,
			String dependency, String dependencyValue, URL mapping)
			throws Exception {
		try {
	...
	/**
	 * Constructor for a property with a single value and according further
	 * informations. Any of the arguments may be null except for the name of the
	 * property.
	 * 
	 * @param name
	 *            {@link String}: the name of the new property, mandatory
	 ...
	 *            {@link String}
	 * @param value
	 *            {@link String}: the value the property shall contain,
	 *            mandatory except for terminologies
	 * @param unit
	 *            {@link String}
	 * @param uncertainty
	 ...
	 *             the exception
	 */
	public Property(String name, String reference, Object value, String unit,
			Object uncertainty, String type, String filename,
			String definition, String valueDefinition, String dependency,
		...
		if (type.equalsIgnoreCase("NULL")) {
			...
			try {
				...
				theValues.add(new Value(value, unit, uncertainty, type,
						filename, valueDefinition, reference));
				initialize(name, theValues, definition, dependency,
						dependencyValue, mapping);
			} catch (Exception l) {
				logger.error("Could not create property: ",
	...
	/**
	 * Constructor for a property with more than one value and the according
	 * other informations. Any of the arguments may be null except for the name
	 * of the property.
	 * 
	 * @param name
	 *            {@link String} The name of the property.
	 * @param values
	 *            {@link Vector} that will be stored in the values.
	 * @param references
	 *            {@link Vector} of Strings, the references of the Values
	 * @param unit
	 *            {@link String}: The unit of the values.
	 * @param uncertainties
	 *            {@link Vector}<Object> vector of value uncertainties.
	 * @param type
	 *            {@link String} value data type. There is only one type since
	 *            mixing different types is not allowed.
	 * @param filenames
	 *            {@link Vector}<Object>: vector if filenames
	 * @param valueDefinitions
	 *            {@link Vector}<String>: Vector of value definitions.
	 * @param definition
	 *            {@link String} The property definition
	 * @param dependency
	 *            {@link String} Property dependence.
	 * @param dependencyValue
	 *            {@link String} the value the dependence Property should have.
	 * @param mapping
	 *            {@link URL} A url to a terminology to which this property
	 *            should be mapped, if needed.
	 * @throws Exception
	 *             the exception
	 */
	public Property(String name, Vector<Object> values,
			Vector<String> references, String unit,
			Vector<Object> uncertainties, String type,
			Vector<String> filenames, Vector<String> valueDefinitions,
			String definition, String dependency, String dependencyValue,
		...
		} else if (values.size() < uncertainties.size()
		...
		for (int i = 0; i < values.size(); i++) {
			...
			try {
				Value value = new Value(values.get(i), unit, tmpUncertainty,
						type, tmpFilenames, tmpComment, tmpReference);
				theValues.add(value);
			} catch (Exception e) {
				logger.error("Error during creation of value: ",
	...
	/**
	 * Called by the Property-constructor to initialize the given components,
	 * i.e. checking for isEmpty or ==null (doing nothing when so except for
	 * mandatory name, there throwing error)
	 * 
	 * @param name
	 *            {@link String} : the Property name.
	 * @param values
	 *            {@link Vector}<Object>: A Vector of values.
	 * @param definition
	 *            the definition
	 * @param dependency
	 *            String
	 ...
	 *             the exception
	 */
	private void initialize(String name, Vector<Value> values,
			String definition, String dependency, String dependencyValue,
			URL mapping) throws Exception {
		if (name.contains("/")) {
	...
	/**
	 * Called by the Property-constructor to initialize as a NULL property.
	 * 
	 * @param name
	 *            {@link String} : the Property name.
	 * @throws Exception
	...
	/**
	 * Set the definition of this property, i.e. the definition that tells what
	 * this property means. Can also be used for removing the definition by
	 * passing null.
	 * 
	 * @param definition
	...
	public void setDefinition(String definition) {
		if (isNullProperty()) {
			logger.warn("Trying to set definition of a NULL property.");
			return;
		}
		if (definition != null)
	...
	public String getDefinition() {
		if (isNullProperty()) {
			logger.warn("Trying to get definition of a NULL property");
		}
	...
	public String getDependency() {
		if (isNullProperty()) {
			logger.warn("Trying to get dependency of a NULL property");
		}
	 ...
	 * 
	 * @return - {@link String}: the value, the dependency property has to
	 *         assume to make this property meaningful.
	 */
	public String getDependencyValue() {
		if (isNullProperty()) {
			logger.warn("Trying to get dependency value of a NULL property");
		}
	...
	/**
	 * Returns the content of the first value as {@link Float}. If content
	 * cannot be converted to float Float.NaN is returned.
	 * 
	 * @return {@link Float} the converted contend, or Float.NaN if conversion
	 *         fails.
	 */
	...
	/**
	 * Returns the content of the i-th value as {@link Float}. If the content
	 * cannot be converted to float Float.NaN is returned.
	 * 
	 * @param i
	...
	public double getNumber(int i) {
		if (isNullProperty()) {
			logger.warn("Trying to getNumber of a NULL property");
			return Double.NaN;
		}
		DecimalFormat myDF = new DecimalFormat();
		myDF.setMaximumFractionDigits(15);
		myDF.setMinimumFractionDigits(2);
		myDF.setDecimalFormatSymbols(DecimalFormatSymbols
	...
	public String getText(int i) {
		if (isNullProperty()) {
			logger.warn("Trying to getText of a NULL property");
			return null;
		}
		try {
	...
	/**
	 * Returns the date component of first value's content if possible. Null
	 * otherwise.
	 * 
	 * @return {@link Date} the date component if possible (yyyy-MM-dd format).
	 *         Null, otherwise.
	 */
	...
	/**
	 * Returns the date component of i-th value's content if possible. Null
	 * otherwise.
	 * 
	 * @param i
	 *            {@link Integer} the value index.
	...
	public LocalDate getDate(int i) {
		if (isNullProperty()) {
			logger.warn("Trying to getDate of a NULL property");
			return null;
		}
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		try {
			return new LocalDate(sdf.parse(getValue(i).toString()));
		} catch (Exception e) {
	...
	/**
	 * Returns the time component of first value's content if possible. Null
	 * otherwise.
	 * 
	 * @return {@link Date} the time component if possible (HH:mm:ss format).
	 *         Null, otherwise.
	 */
	...
	/**
	 * Returns the time component of i-th value's content if possible. Null
	 * otherwise.
	 * 
	 * @param i
	 *            {@link Integer} the value index.
	...
	public LocalTime getTime(int i) {
		if (isNullProperty()) {
			logger.warn("Trying to getTime of a NULL property");
			return null;
		}
		SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
		try {
			return new LocalTime(sdf.parse(getValue(i).toString()));
		} catch (Exception e) {
	...
	/**
	 * Returns the datetime of first value's content if possible. Null
	 * otherwise.
	 * 
	 * @return {@link Date} the date component if possible (yyyy-MM-dd HH:mm:ss
	 ...
	 */
	public boolean addValue(Object value, String id, Object uncertainty,
			String filename, String definition) {
		return addValue(value, id, null, uncertainty, null, filename,
				definition);
	}
	...
	/**
	 * Adds a new Value-class instance to this property. The function refuses
	 * adding if the value already exists in the property. All arguments except
	 * the value can be null or empty. Method also checks whether type of value
	 * to add is same, if not throwing warning as not known which type is
	 * correct as already existing ones
	 * 
	 * @param value
	 ...
	 *            {@link String}
	 * @param filename
	 *            {@link String}: the default filename which is only of use when
	 *            binary data is stored.
	 * @param comment
	 *            {@link String}: the definition of this value. * @return
	 *            {@link Boolean}: true if the operation succeeded. False if the
	 *            value is null, or the value already exists in the property.
	 * @return true, if successful
	 */
	public boolean addValue(Object value, String id, String unit,
			Object uncertainty, String type, String filename, String comment) {
		if (isNullProperty()) {
			logger.warn("Trying to add value to a NULL property");
			return false;
		}
		if (value == null) {
		...
		try {
			...
			Value toAdd = new Value(value, unit, uncertainty, type, filename,
					comment, id);
			...
			values.add(toAdd);
			if (type != null && (!type.isEmpty())) {
				if ((values.get(0).getType() != null)
						&& (!values.get(0).getType().isEmpty())
				...
				} else if ((values.get(0).getType() == null)
						|| (values.get(0).getType().isEmpty())) {
					this.setType(type);
				}
			}
	...
	/**
	 * Adds new Values (i.e. instances of class) to the property. Function
	 * appends all values stored within the passed property.
	 * 
	 * @param property
	...
	public void addValue(Property property) {
		if (isNullProperty()) {
			logger.warn("Trying to add value to a NULL property. No values added.");
			return;
		}
	 ...
	 * @param value
	 *            {@link Object}; the new value = value-content.
	 * @return {@link Boolean} true if operation succeeded, false if more than
	 *         on value = value-content is stored.
	 */
	...
	/**
	 * Set the value = value-content at a specific position in the values
	 * vector.
	 * 
	 * @param value
	...
	public boolean setValueAt(Object value, int index) {
		if (isNullProperty()) {
			logger.warn("Trying to set value of a NULL property. Value ignored.");
			return false;
		}
		...
		try {
			...
			logger.info("Property.setValueAt: successfully set value at index "
					+ index);
		} catch (Exception e) {
	...
	public int valueCount() {
		if (isNullProperty()) {
			logger.warn("NULL property cannot contain any values.");
			return -1;
		}
	 ...
	 * 
	 * @param value
	 *            {@link Value}: the value of which the index should be
	 *            returned.
	 * @return {@link Integer}: the values index or -1 if the value does not
	 *         exist.
	 */
	public int getValueIndex(Object value) {
		if (isNullProperty()) {
			logger.warn("Trying query NULL property for value index.");
		}
		for (int i = 0; i < values.size(); i++) {
	...
	public Vector<Object> getValues() {
		if (isNullProperty()) {
			logger.warn("NULL property does not contain any values.");
			return null;
		}
	...
	public Object getValue(int index) {
		if (isNullProperty()) {
			logger.warn("NULL property does not contain any value.");
			return null;
		}
		try {
	...
	/**
	 * Get a certain value of class Value (meaning with all it's details)
	 ...
	 * @param index
	 *            {@link Integer}: the index.
	 * @return {@link Boolean}: true if removal was successful, false if index
	 *         out of bounds.
	 */
	...
	/**
	 * Returns whether or not this Property is empty. A Property is empty if it
	 * does not have any values or if all existing values have no content.
	 * 
	 * @return boolean
	 */
	 ...
	 * @return {@link Boolean} true if operation succeeded, false if there is
	 *         more than a single value stored in this property or is a NULL
	 *         property.
	 */
	public boolean setValueReference(String reference) {
		...
		return setValueReferenceAt(reference, 0);
	}
	...
	/**
	 * Set the value id of a value specified by its index. Overwrites old
	 * information.
	 * 
	 ...
	 * @param index
	 *            {@link Integer}: the index of the value to which the id
	 *            belongs.
	 * @return {@link Boolean}: true if new id was set, false if index out of
	 *         bounds.
	 */
	@Deprecated
	...
	/**
	 * Set the value reference of a value specified by its index. Overwrites old
	 * information.
	 * 
	 * @param reference
	 *            {@link String}: the new value id.
	 ...
	 *            belongs.
	 * @return {@link Boolean}: true if new id was set, false if index out of
	 *         bounds or NULL property.
	 */
	public boolean setValueReferenceAt(String reference, int index) {
		if (isNullProperty()) {
			logger.warn("Trying to set value reference for a NULL property.");
			return false;
		}
	...
	/**
	 * Get the value ids. Funciton is marked @deprecated and will be removed.
	 * 
	 * @return {@link Vector} of Strings. the values. This vector contains empty
	 *         strings if no ids stored.
	 */
	@Deprecated
	 ...
	 * 
	 * @return {@link Vector} of Strings. the values. This vector contains empty
	 *         strings if no references stored.
	 */
	public Vector<String> getValueReferences() {
		if (isNullProperty()) {
			logger.warn("Trying to get value references for a NULL property.");
			return null;
		}
	 ...
	 * @return {@link String}: the id stored for the value, an empty string in
	 *         none stored or null if the index is out of bounds.
	 */
	public String getValueReference(int index) {
		if (isNullProperty()) {
			logger.warn("Trying to get value reference for a NULL property.");
			return null;
		}
		try {
			...
			logger.error(
					"! fetching id for given value specified by it's index failed: ",
					e);
			return null;
		}
	}
	 ...
	 * @return {@link Boolean} true if operation succeeded, false if there is
	 *         more than a single value stored in this property.
	 */
	...
	/**
	 * Set the error of a value specified by its index. Overwrites old
	 * information.
	 * 
	 * @param uncertainty
	 *            {@link Object}: the new error estimate.
	 * @param index
	 *            {@link Integer}: the index of the value to which the error
	 *            belongs.
	 * @return {@link Boolean}: true if new error was set, false if index out of
	 *         bounds.
	 */
	public boolean setValueUncertaintyAt(Object uncertainty, int index) {
		if (isNullProperty()) {
			logger.warn("Trying to set value uncertainity for a NULL property.");
			return false;
		}
		if (this.values.size() <= index || index < 0) {
	...
	public Vector<Object> getValueUncertainties() {
		if (isNullProperty()) {
			logger.warn("NULL property has no values and hence no value uncertainities.");
			return null;
		}
		Vector<Object> toReturn = new Vector<Object>();
		for (int i = 0; i < this.values.size(); i++) {
	...
	/**
	 * Get the uncertainty estimation stored for the value. Only working if
	 * there is only one value!
	 * 
	 * @return {@link Object}: the according uncertainty value if existing. An
	 *         empty String if no error stored or null if there are more values
	 *         existing
	 */
	public Object getValueUncertainty() {
		if (isNullProperty()) {
			logger.warn("NULL property has no value and hence no value uncertainity.");
			return null;
		}
		if (values.size() > 1) {
	...
	/**
	 * Get the uncertainty estimation stored for a certain value defined by its
	 * index.
	 * 
	 * @param index
	 *            {@link Integer}: the index in the values vector.
	 * @return {@link Object}: the according uncertainty value if existing. An
	 *         empty String if no error stored or null if the index is out of
	 *         range.
	 */
	...
	public boolean setValueDefinition(String definition) {
		if (isNullProperty()) {
			logger.warn("Trying to set value definition for a NULL property.");
			return false;
		}
		if (this.values.size() > 1) {
	...
	/**
	 * Set the value definition of a value specified by its index. Overwrites
	 * old information.
	 * 
	 * @param definition
	 *            {@link String}: the new value definition.
	 * @param index
	 *            {@link Integer}: the index of the value to which the
	 *            definition belongs.
	 * @return {@link Boolean}: true if new definition was set, false if index
	 *         out of bounds.
	 */
	...
	public Vector<String> getValueDefinitions() {
		if (isNullProperty()) {
			logger.warn("NULL propertyhad no values and hence no value definitions.");
			return null;
		}
		Vector<String> toReturn = new Vector<String>();
		for (int i = 0; i < values.size(); i++) {
	 ...
	 * 
	 * @param index
	 *            {@link Integer} the index of the value, respectively its
	 *            defintion.
	 * @return {@link String}: the value comment if stored, an empty string if
	 *         not or null if the index is out of bounds.
	 */
	...
	/**
	 * Merges this property with another one. Generally, all values and other
	 * information of the other property will be copied. The way the merging is
	 * done in case of conflict can be set by the mergeOption parameter which
	 * can assume the following values:
	 * <ol>
	 * <li>MERGE_THIS_OVERRIDES_OTHER: the local information overrides the
	 * information passed in otherProperty.</li>
	 * <li>MERGE_OTHER_OVERRIDES_THIS: the global information passed with
	 * otherProperty overrides local information.</li>
	 * <li>MERGE_COMBINE: the local information will be combined with the passed
	 * property. values with the same name will be fused.</li>
	 * </ol>
	 * 
	 * @param otherProperty
	 *            {@link Property} the other Property which shall be merged with
	 *            this property.
	 * @param mergeOption
	...
	public void merge(Property otherProperty, int mergeOption) {
		...
		if ((this.getDefinition() != null && otherProperty.getDefinition() != null)
				&& !this.getDefinition().equalsIgnoreCase(
		...
		// actually merge: first the easy ones, i.e. those that can occur only
		// once
		...
		if (this.getDependency() == null) {
			...
			if (mergeOption == Section.MERGE_OTHER_OVERRIDES_THIS
					&& (otherProperty.getDependency() != null || otherProperty
							.getDependency().isEmpty())) {
				this.setDependency(otherProperty.getDependency());
			}
		}
		if (this.getDependencyValue() == null) {
			...
			if (mergeOption == Section.MERGE_OTHER_OVERRIDES_THIS
					&& (otherProperty.getDependencyValue() != null || otherProperty
							.getDependency().isEmpty())) {
				this.setDependency(otherProperty.getDependency());
			}
		}
		for (int i = 0; i < otherProperty.valueCount(); i++) {
			if (this.values.contains(otherProperty.getValue(i))) {
				mergeValue(this.values.indexOf(otherProperty.getValue(i)),
						otherProperty, i, mergeOption);
			} else {
				if (mergeOption == Section.MERGE_COMBINE) {
					this.addValue(otherProperty.getValue(i),
							otherProperty.getValueReference(i),
							otherProperty.getValueUncertainty(i),
							otherProperty.getFilename(i),
							otherProperty.getValueDefinition(i));
				} else if (mergeOption == Section.MERGE_OTHER_OVERRIDES_THIS
						&& this.valueCount() == 1) {
	 ...
	 *            the other property
	 */
	public void convertProperty(Property otherProperty) {
		if (otherProperty.isNullProperty()) {
			logger.error("Trying to convert a NULL property to another NULL property.");
			return;
		}

		try {
			Vector<Value> theValues = new Vector<Value>();
			theValues.add(new Value(otherProperty.getValue(), null));
			initialize(otherProperty.getName(), theValues, null, null, null,
					null);
		} catch (Exception l) {
			logger.error("Could not create property: ", l.getLocalizedMessage());
		}
	 ...
	 *            The {@link Property} as it is defined in the terminology.
	 */
	public void validate(Property terminologyProperty) {
		if (definition != null && !definition.isEmpty()) {
			if (!this.definition.equalsIgnoreCase(terminologyProperty
					.getDefinition())) {
		...
		if (terminologyProperty.getDependency() != null
			...
			if (this.getParent() != null
					&& !this.getParent().containsProperty(
				...
				if (terminologyProperty.getDependencyValue() != null
					...
					while (iter.hasNext()) {
						if (iter.next().toString()
								.equalsIgnoreCase(terminolgyValue)) {
							match = true;
							break;
						}
					}
	...
	/**
	 * Merges a value of a property and all its appendant elements. Again,
	 * information that exists in only one of the properties are accepted
	 * irrespective of the mergeOption. Only if conflicts arise the mergeOption
	 * is relevant.
	 * 
	 ...
	 * @param mergeOption
	 *            {@link Integer} defines the behavior if a conflict arises, see
	 *            {@linkplain Property.merge}
	 */
	private void mergeValue(int thisValueIndex, Property otherProperty,
			int otherValueIndex, int mergeOption) {
		switch (mergeOption) {
			case Section.MERGE_THIS_OVERRIDES_OTHER:
				if (this.getValueDefinition(thisValueIndex) == null) {
					this.setValueDefinitionAt(
							otherProperty.getValueDefinition(otherValueIndex),
							thisValueIndex);
				}
				if (this.getValueUncertainty(thisValueIndex) == null) {
					this.setValueUncertaintyAt(
							otherProperty.getValueUncertainty(otherValueIndex),
							thisValueIndex);
				}
				if (this.getValueFilename(thisValueIndex) == null) {
					this.setValueFilenameAt(
							otherProperty.getValueFilename(otherValueIndex),
							thisValueIndex);
				}
				if (this.getValueReference(thisValueIndex) == null) {
					this.setValueReferenceAt(
							otherProperty.getValueReference(otherValueIndex),
							thisValueIndex);
				}
				break;
			...
			case Section.MERGE_COMBINE:
				if (this.getValueDefinition(thisValueIndex) == null) {
					...
					this.setValueDefinitionAt(
							this.getValueDefinition(thisValueIndex)
									+ "\n"
									+ otherProperty
											.getValueDefinition(otherValueIndex),
							thisValueIndex);
				}
				if (this.getValueUncertainty(thisValueIndex) == null) {
					this.setValueUncertaintyAt(
	...
	/**
	 * Checking name and if necessary converting to CamelCase leading characters
	 * that are not alphabetical get P_ at the beginning.
	 * 
	 * @param name
	...
	/**
	 * Returns the property as an {@link Vector} of Objects. If there is more
	 * than one value, the first is returned. "name","value"
	 * 
	...
	/**
	 * Returns the property as an Vector of Objects. The index of the value must
	 * be given.
	 * 
	 * @param index
	 *            {@link Integer}: the index of the selected value.
	 * @return Vector<Object> the content of this property and the specified
	 *         value. If index out of bounds null is returned.
	 */
	...
	/**
	 * Get the property's unit. If there is more than one value the index must
	 * be specified to know which unit shall be returned, otherwise null. units
	 * can differ like the following example: sec, millisec, nanosec
	 * 
	 * @return {@link String}
	 */
	...
	/**
	 * Get the value's data type. As all types must be the same, type of first
	 * value is returned.
	 * 
	 * @return {@link String}
	 */
	...
	/**
	 * Function is marked @deprecated and will be removed. Use
	 * getValueFilename() instead!
	 * 
	 * @return the filename
	 */
	@Deprecated
	...
	/**
	 * Function is marked @deprecated and will be removed. Use
	 * getValueFilename(int index) instead!
	 * 
	 * @param index
	 *            the index
	 * @return the filename
	...
	/**
	 * Returns the mimetype of the property. Only working if not more than one
	 * value existing!
	 * 
	 * @return {@link String}: returns the mimetype of a certain propery.
	 *         Returns an empty String if no mimeType stored or null if the
	 *         index is out of range.
	 */
	public String getValueFilename() {
		if (values.size() > 1) {
			logger.error("! property has more than one value > index for returning filename must be specified");
			return null;
		}
		return getValueFilename(0);
	}
	...
	/**
	 * Returns the filename information stored in the specified value defined by
	 * the index.
	 * 
	 * @param index
	 *            {@link Integer}: the index to specify which filename shall be
	 *            returned (i.e. from which value)
	 ...
	 * 
	 * @return String the checksum or null if no checksum is stored or index out
	 *         of bounds.
	 */
	...
	/**
	 * Checks if is null property.
	 * 
	 * @return true, if is null property
	 */
	...
	/**
	 * Set the mapping of this {@link Property}. The mapping is specified in
	 * form of an {@link URL} (e.g.
	 * http://portal.g-node-org/odml/terminologies/sectionType#newPoperty). The
	 * anchor part may be missing. In this case the property will be copied into
	 * a destination section of the specified type.
	 * 
	 * @param mappingURL
	 *            an {@link URL} defining
	 */
	...
	/**
	 * Removes the mapping information from this property. The parent mapping,
	...
	/**
	 * Careful! Sets all units of this property (i.e. of all values) to the same
	 * content! Overwrites old content!
	 * 
	 * @param unit
	 ...
	 * 
	 * @return {@link URL} the mapping information related to this property or
	 *         null if none found.
	 */
	...
	/**
	 * Setting the unit of the value specified by an index to the new unit;
	...
	/**
	 * Set the parent of this property. Parents indicate that this property is
	 * only meaningful once the parent property is specified.Overwirtes old
	 * content!
	 * 
	 * @param dependency
	...
	/**
	 * Set the reference of this property. Only working if not more than one
	 * value existing!
	 * 
	 * @param reference
	...
	public boolean setReference(String reference) {
		if (this.values.size() > 1) {
			logger.error("! Property has more than one value > index must be specified to know which reference shall be set!");
			return false;
		}
	...
	/**
	 * Set the value filename of a value specified by its index. Overwrites old
	 * information.
	 * 
	 * @param filename
	 *            {@link String}: the default file name which should be used
	 *            when saving the object.
	 * @param index
	 *            {@link Integer}: the index of the value to which the filename
	 *            belongs.
	 * @return {@link Boolean}: true if new name was set, false if index out of
	 *         bounds.
	 * 
	...
	/**
	 * Set the filename associated with the binary content of value specified by
	 * index. Overwrites old information.
	 * 
	 * @param filename
	 *            {@link String}: the default file name which should be used
	 *            when saving the object.
	...
	/**
	 * Writes the value of this property (if of type binary) to disc. In case
	 * there is more than one value stored, the first is written to disc.
	 * 
	 * @param filename
	...
	public void writeBinaryToDisc(URI fileUri) {
		try {
			...
			logger.error("could not create File from the specified URI: "
					+ fileUri, e);
		}
	}
	...
	/**
	 * Write the property content to disc. Function calls public method
	 * writeBinaryToDisc.
	 * 
	 * @param outFile
	...
	private void writeBinary(File outFile, int index) throws Exception {
		...
		if (outFile.exists()) {
			throw new Exception(
					"File already exists please provide a different file name.");
		}
		Value.writeBinaryToDisc(values.get(index).getContent().toString(),
				outFile);
	}

	/**
	 * Returns a String representation of this section. I.e the fact that it is
	 * a section and it's name.
	 * 
	 * @return the string
	 */
	@Override
	...
	/**
	 * Returns an extended String representation of this section. I.e it's name,
	 * level, completePath, number and names of subsections and number and names
	 * of appended properties
	 * 
	 * @return the string
	 */
	...
	public static int match(Object anObject, Object anotherObject, String type) {
		if (anObject == null || anotherObject == null || type == null) {
			System.out
					.println("match returns error, object1, object2 or type is null.");
			return MATCH_ERROR;
		}
		if (type.isEmpty()) {
	...
	/**
	 * Tries to match two strings that represent names. Returns a match score
	 * between MATCH_NO (0) and MATCH_FIRST_LAST (50). Method breaks down the
	 * names into first and last name component. Middle initials and second
	 * names are ignored. Pure initials work only if both names are initials. If
	 * names consist of one word only it is assumed to be the last name. Match
	 * levels are:
	 * <ol>
	 * <li><b>MATCH_ERROR:</b> if the some error occurred, e.g. if arguments are
	 * invalid.</li>
	 * <li><b>MATCH_NO:</b> if no match was found.</li>
	 * <li><b>MATCH_FIRST_CONFLICT_LAST_MATCH:</b> if the last name matches but
	 * the first name fails. Might be the consequence of a misspelling.</li>
	 * <li><b>MATCH_FIRST_OR_LAST_ONLY:</b> if at least one name has only first
	 * or last name component matching can at best be a first or last name match
	 * alone.</li>
	 * <li><b>MATCH_FIRST_INITIAL_LAST:</b> if the last matches but the first
	 * matches only with the initials or the staring sequence. This may occur if
	 * a first name like "Jean-Marc" is passed as "Jean Marc" or with only first
	 * or middle name alone.</li>
	 * <li><b>MATCH_FIRST_LAST:</b> the best score, only in there are first and
	 * last name given and both are matching. Some risk resides if persons are
	 * distinguished with their middle names/initials only./</li>
	 * 
	 * </ol>
	 * 
	 * @param name1
	...
	private static int nameMatch(String name1, String name2) {
		if (name1 == null || name2 == null) {
			System.out
					.println("nameMatch return error, one of the names is null");
			return MATCH_ERROR;
		}
		if (name1.isEmpty() || name2.isEmpty()) {
			System.out
					.println("nameMatch return error, one of the names is empty");
			return MATCH_ERROR;
		}
		...
		if (!firstName1.isEmpty() && !firstName2.isEmpty()) {
			if (firstName1.equalsIgnoreCase(firstName2)
				...
				return MATCH_FIRST_LAST;
					...
					&& (firstName1.startsWith(firstName2) || firstName2
							.startsWith(firstName1))
					&& lastName1.equalsIgnoreCase(lastName2)) {
				return MATCH_FIRST_INITIAL_LAST;
			} else if ((firstName1.startsWith(firstName2) && lastName1
					.startsWith(lastName2))
					|| (firstName2.startsWith(firstName1) && lastName2
							.startsWith(lastName1))) {
	...
	public static String getLastName(String fullName) {
		if (fullName.contains(",")) {// assuming lastName first then firstName
										// maybe as initial or with middle name
			// initial
			return fullName.substring(0, fullName.indexOf(","));
		} else if (!fullName.contains(".") && fullName.contains(" ")) {// firstName
																		// first
																		// no
																		// initial,
																		// lastName
		...
		} else if (fullName.contains(".") && !fullName.contains(" ")) {// has an
																		// initial,
																		// assume
																		// that
																		// the
			...
			// behind the dot.
			return fullName.substring(fullName.lastIndexOf(".") + 1);
		} else if (!fullName.contains(",") && !fullName.contains(" ")) {// is
																		// only
																		// one
																		// word,
																		// assumes
			...
			// a last name
			return fullName;
		} else {// firstName first, maybe as initial or with intials, lastName
				// last
			return fullName.substring(fullName.lastIndexOf(" ") + 1);
		}
	}
	...
	public static String getFirstName(String fullName) {
		if (fullName.contains(",")) {// assuming lastName first then firstName
			...
			String firstName = fullName.substring(fullName.indexOf(",") + 1)
					.trim();
			if (firstName.contains(" ")) {// maybe more than one first name,
											// take only the first
				firstName = firstName.substring(0, firstName.indexOf(" "));
			}
			if (firstName.contains(".")) {// take only the firstName or fistName
											// initial without the dot
				firstName = firstName.substring(0, firstName.indexOf("."));
			}
			return firstName;
		} else if (fullName.contains(".") && !fullName.contains(" ")) {// firstName
	...
	public int hashCode() {
		...
		result = prime * result
				+ ((dependencyValue == null) ? 0 : dependencyValue.hashCode());
		result = prime * result
				+ ((mappingURL == null) ? 0 : mappingURL.hashCode());
		result = prime * result + ((name == null) ? 0 : name.hashCode());

		// cannot use parentSection - would cause infinite loop
		// result = prime * result + ((parentSection == null) ? 0 :
		// parentSection.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (dependencyValue == null) {
			if (other.dependencyValue != null) {
				return false;
			}
		} else if (!dependencyValue.equals(other.dependencyValue)) {
			return false;
		}
		if (mappingURL == null) {
		...
		// cannot use parentSection - would cause infinite loop
		/*
		 * if (parentSection == null) { if (other.parentSection != null) {
		 * return false; } } else if
		 * (!parentSection.equals(other.parentSection)) { return false; }
		 */
