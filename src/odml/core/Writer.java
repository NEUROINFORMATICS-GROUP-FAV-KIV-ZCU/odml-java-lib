...
/**
 * The {@link Writer} class provides the tools to write odML metadata files.
 * 
...
public class Writer implements Serializable {
	...
	/**
	 * Creates a writer instance. Lets the Wirter write only those properties
	 * that have values.
	 * 
	 * @author Jakub Krauz
	 * @param rootSection
	 ...
	 * write also those properties that have no values as is usually the case
	 * for terminologies.
	 * 
	 * @author Jakub Krauz
	 * @param rootSection
	 *            {@link Section}: the rootSection of the odml metadata tree.
	 * @param asTerminology
	 *            {@link Boolean}: if true also emtpy properties (no value) are
	 ...
	 * 
	 * @param filename
	 *            {@link String} the full name of the destination file
	 *            (including path).
	 * @param rootSection
	 *            {@link Section} the root Section of the metadata tree.
	 */
	...
	/**
	 * Creates a Writer-instance. Writes only non-empty properties into the
	 * metadata files.
	 * 
	 * @param file
	 *            {@link File} the File into which the metadata should be
	 *            written.
	 * @param rootSection
	 *            {@link Section}: the rootSection of the odml metadata tree.
	 */
	 ...
	 * @param asTerminology
	 *            {@link Boolean}: if true also emtpy properties (no value) are
	 *            written, otherwise only non-emty properties are written to
	 *            disc.
	 */
	 ...
	 * @author Jakub Krauz
	 * @param stream
	 *            {@link OutputStream}: output stream to which to write the
	 *            document
	 * @return {@link Boolean} true if operation was successful, false
	 *         otherwise.
	 */
	...
	/**
	 * Writes the odML serialization to the given output stream. The odML tree
	 * can be optimized (linked sections are simplified to reduce redundancy)
	 * and validated against terminologies before the serialization.
	 * 
	 * @author Jakub Krauz
	 * @param stream
	 *            {@link OutputStream}: output stream to which to write the
	 *            document
	 * @param optimize
	 *            {@link Boolean}: remove empty properties and sections, removes
	 *            redundancy in linked sections.
	 * @param validate
	 *            {@link Boolean}: validates the metadata against the
	 *            terminologies.
	...
	/**
	 * Write the metadata to disc after the tree has been optimized (linked
	 * sections are simplified to reduce redundancy) and validated against the
	 * terminologies.
	 * 
	 * @param optimize
	 *            {@link Boolean}: remove empty properties and sections, removes
	 ...
	 * 
	 * @param odMLRoot
	 *            {@link Section}: the section to start the dom creation.
	 * @param asTerminology
	 *            {@link boolean}: flag to indicate whether Template is used or
	 *            not
	 * @return {@link boolean}: true if creating Dom successfully, otherwise
	 *         false
	 */
	 ...
	 * 
	 * @param parent
	 *            {@link Element}: the parent where the section shall be
	 *            appended
	 * @param section
	 *            {@link Section}: the section to append to the parent-element
	 * @param asTemplate
	 *            {@link boolean}: flag to indicate whether template or not; if
	 *            template then also writing value-information (e.g. unit or
	 *            type) without having actual value-content
	 */
	private void appendSection(Element parent, Section section,
			boolean asTemplate) {
	...
	/**
	 * Appends a property elements to the dom tree. Empty properties (those with
	 * no values) will only be written to file if the file is to become a
	 * terminology.
	 * 
	 * @param parent
	 *            {@link Element}: the parent Element to which the properties
	 *            belong.
	 * @param prop
	 *            {@link Property}: the property to append.
	 * @param asTerminology
	 *            boolean: defines whether the file will be a terminology.
	 */
	private void appendProperty(Element parent, Property prop,
			boolean asTerminology) {
	 ...
	 * 
	 * @param parent
	 *            {@link Element}: the parent Element to which the values
	 *            belong.
	 * @param val
	 *            the val
	 * @param asTemplate
	...
	private void appendValue(Element parent, Value val, boolean asTemplate) {
		if (!asTemplate) {
			if (val.getContent() == null/*
										 * ||
										 * val.getContent().toString().isEmpty()
										 */) {
				return;
			}
		}
	 ...
	 * @param stream
	 *            the output stream
	 * @return true if the dom tree was successfully written to the stream,
	 *         false otherwise
	 */
	...
	private boolean streamToFile(File newFile) {
		try {
			...
			org.jdom.output.Format frmt = org.jdom.output.Format
